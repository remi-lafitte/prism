...
set.seed(123)
#load the data
meta <- read.csv(here::here("VV_META","VV_META.csv"), sep = ";", dec = ",")
meta$age_c<-ifelse(meta$age == "young", -0.5, 0.5)
#CORRELATION---------
# identify moderators
cor(meta[c("age_c", "d", "SD", "SE", "deg", "trial")])
# there is no obvious moderators.
# age acts on SD, but not on VV bias


#DEGREE META---------------
# defining the priors
prior1 <- c(
  prior(normal(0, 2), class = Intercept), # degree' prior
  prior(cauchy(0, 2), class = sd)
)
# modelisation
mod_degree <- brm(
  deg | se(SD) ~ 1 + (1|ARTICLE) + (1|ID),
  data = meta,
  prior = prior1,
  sample_prior = FALSE,
  save_all_pars = TRUE,
...
# result
plot(mod_degree, combo = c("dens_overlay", "trace"), 
...
# defining the priors
prior_d <- c(
  prior(normal(0, 2), class = Intercept), # d prior
  prior(cauchy(0, 2), class = sd)
)
# modelisation
mod_d <- brm(
  d | se(SD) ~ 1 + (1|ARTICLE) + (1|ID),
  data = meta,
  prior = prior_d,
  sample_prior = FALSE,
  save_all_pars = TRUE,
...
f<-
  ...
  labs(y = "Article", x = "Overall effect size (Cohen's d)")
f
...
hist(meta$SD)
# we see two outliers
meta2<-meta[meta$ARTICLE != "Braem (2014)",] 
x1 <- meta2$SD                   
w1 <- meta2$n 
weighted.mean(x1, w1)   

# quid age ?
meta3<-meta2[meta2$age == "young",]
x1 <- meta3$SD                   
w1 <- meta3$n 
weighted.mean(x1, w1)  
